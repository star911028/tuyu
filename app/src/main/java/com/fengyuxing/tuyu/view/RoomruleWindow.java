...
public class RoomruleWindow extends PopupWindow {
    ...
    public RoomruleWindow(final Activity context, OnClickListener l, Boolean ismanger,String Description) {
        this.context = context;
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        conentView = inflater.inflate(R.layout.rule_window, null);
        close_iv = (ImageView) conentView.findViewById(R.id.close_iv);
        edit_tv = (TextView) conentView.findViewById(R.id.edit_tv);
        rule_tv= (TextView) conentView.findViewById(R.id.rule_tv);
        pop_layout = (LinearLayout) conentView.findViewById(R.id.pop_layout);
        close_iv.setTag(4);
        close_iv.setOnClickListener(l);
        edit_tv.setTag(5);
        edit_tv.setOnClickListener(l);

        if(Description.length()<1){
            rule_tv.setText("暂无说明");
        }else {
            rule_tv.setText(Description);
        }
        if (ismanger) {
