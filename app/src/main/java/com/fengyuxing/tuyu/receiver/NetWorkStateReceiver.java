...
import com.fengyuxing.tuyu.util.NetUtils;

/**
 * Created by LGJ on 18/06/25.
 */
public class NetWorkStateReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        int netWork = NetUtils.getNetWorkState(context);
        if(netWork == NetUtils.NETWORK_MOBILE){
            Toast.makeText(context, "移动数据已连接", Toast.LENGTH_SHORT).show();
        }else if(netWork == NetUtils.NETWORK_WIFI){
            Toast.makeText(context, "WIFI已连接", Toast.LENGTH_SHORT).show();
        }else if(netWork == NetUtils.NETWORK_NONE){
