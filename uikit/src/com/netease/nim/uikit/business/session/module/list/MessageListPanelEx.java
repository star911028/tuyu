...
public class MessageListPanelEx {
    ...
    private OnItemClickListener listener = new OnItemClickListener() {
        @Override
        public void onItemClick(IRecyclerView adapters, View view, int position) {

//            Log.e("Message", "onItemClick" + adapter.getItem(position).getSessionId());
//            Intent intent = new Intent("com.nangch.broadcasereceiver.MYRECEIVER");
//            intent.putExtra("sessionId", adapter.getItem(position).getSessionId());
//            intent.putExtra("sessionName", adapter.getItem(position).getFromNick());
//            container.activity.sendBroadcast(intent);      //发送广播
//            Log.e("BroadcastReceiver", "发送广播" + "   sessionId=" + adapter.getItem(position).getSessionId() + "  sessionName=" + adapter.getItem(position).getFromNick());
        }

        @Override
        public void onItemLongClick(IRecyclerView adapter, View view, int position) {
            Log.e("Message", "onItemLongClick");
        }

        @Override
        public void onItemChildClick(IRecyclerView adapter2, View view, int position) {
            Log.e("Message", "onItemChildClick");
            if (!isSessionMode() || !(view instanceof RobotLinkView)) {
                return;
            }
//              adapter2.getItemViewType(IRecyclerView.HEADER_VIEW)
